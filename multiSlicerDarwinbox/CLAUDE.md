...
### Untouched numeric and time slicers filtered the report (2.5.0.0)
- **Root cause:** a numeric field defaults to `numberType: 'Is less than'` seeded with the
  data maximum, and a time field to `timeCondition: 'Is'`. Both were collected into the
  tuple filter even when the user never touched them, so numeric silently dropped every
  row at the maximum value and time dropped every row in the report.
- **Fix:** `collectNumericSelectedData` and the time branch of `collectDateTimeSelectedData`
  now skip a slicer that is not marked as touched. `Visual.touchedSlicers` holds the query
  names the user has interacted with; it survives the re-render that follows Apply, is
  re-seeded from the applied filter on reopen, and is cleared by Reset.
- **Affected files:** `applyFilter.ts`, `renderNumber.ts`, `renderTimePicker.ts`, `visual.ts`.

### Five-second freeze on large models (2.5.0.0)
- **Root cause:** `getAllIndexes` scanned the whole column once per distinct value and
  accumulated with `[...acc, index]`, so cost scaled with rows x distinct values and had a
  quadratic term. `buildTree` rebuilt the full path-string array for every hierarchy node.
  30,000 rows with a high-cardinality date column took 5,124ms per render.
...
- `renderList.ts` binds `checkboxDivs` with the key `` `${slicer.displayName}-${d.value}}` ``,
  which is not unique for hierarchy nodes that share a label under different parents.
  Tested at 2.5.0.0 with a 3-level hierarchy containing a city name repeated under ten
  different departments: all 38 nodes rendered, so d3 is not dropping rows today. The key
  should still include `d.uid` for correctness, but this is a latent risk, not a live bug.
